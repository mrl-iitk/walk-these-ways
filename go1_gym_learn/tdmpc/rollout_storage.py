...
class RolloutStorage:
    ...
    def sample(self):
        """
        Sample a batch matching Buffer class output exactly.
        Returns:
            tuple: (obs, actions, rewards, None) where:
                obs: shape (horizon+1, batch_size, *obs_shape)
                actions: shape (horizon, batch_size, *action_shape)
                rewards: shape (horizon, batch_size, 1)
                None for task compatibility
        """
        # Calculate valid starting positions
        max_start_idx = self.num_transitions_per_env - (self.cfg.horizon + 1)
        
        # Sample batch_size starting positions
        start_indices = torch.randint(0, max_start_idx, (self.cfg.batch_size,), device=self.device)
        ...
        # Gather observations (including the extra observation)
        for t in range(self.cfg.horizon + 1):
            obs_t = self.observations[start_indices + t]
            obs_list.append(obs_t)
            
        # Gather actions and rewards (excluding the last timestep)
        for t in range(self.cfg.horizon):
            action_t = self.actions[start_indices + t]
            reward_t = self.rewards[start_indices + t]
            action_list.append(action_t)
            reward_list.append(reward_t)
        
        # Stack along time dimension
        obs = torch.stack(obs_list)  # shape: (horizon+1, batch_size, *obs_shape)
        actions = torch.stack(action_list)  # shape: (horizon, batch_size, *action_shape)
        rewards = torch.stack(reward_list)  # shape: (horizon, batch_size, 1)
        
        return obs, actions, rewards, None
