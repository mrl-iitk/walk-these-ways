...
from m2_gym import MINI_GYM_ROOT_DIR
from m2_gym.envs.base.base_task import BaseTask
from m2_gym.utils.math_utils import quat_apply_yaw, wrap_to_pi, get_scale_shift
from m2_gym.utils.terrain import Terrain
...
class LeggedRobot(BaseTask):
    ...
    def _update_terrain_curriculum(self, env_ids, cfg):
        ...
        # Only allow progression if episode was long enough
        # elapsed_time = self.episode_length_buf[env_ids]
        # valid_episode = elapsed_time > cfg.env.episode_length_s

        move_up = (distance > cfg.terrain.env_length*0.6) 
        move_down = (distance < torch.norm(self.commands[env_ids, :2], dim=1)*cfg.env.episode_length_s*0.4) & ~move_up
        # level_up = 1 * move_up - 1 * move_down
        # if level_up>0: level_up = 1
        # elif level_up<0:level_up=-1
        # print(move_down)
        self.terrain_levels[env_ids] += 1 * move_up - 1 * move_down
        # print(self.terrain_levels)
        # torch.clip(self.terrain_levels[env_ids], 0)
        # self.terrain_levels[env_ids] += level_up
        ...
        self.terrain_levels[env_ids] = torch.where(self.terrain_levels[env_ids]>=self.cfg.terrain.max_terrain_level,
                                                   ...
                                                   torch.clip(self.terrain_levels[env_ids], 0))
        # if any(i>3 for i in self.terrain_levels): print(self.terrain_levels)
        # print(self.terrain_levels)
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        self.gym.set_actor_root_state_tensor_indexed(self.sim,
                                                     gymtorch.unwrap_tensor(self.root_states),
                                                     gymtorch.unwrap_tensor(env_ids_int32), len(env_ids_int32))
        # if any(i>5 for i in self.root_states[:,0]): print(self.root_states[:, :3])
    ...
    def _init_buffers(self):
        ...
        if self.cfg.control.control_type == "actuator_net":
            abd_actuator_path = "/home/xterra/walk-these-ways/resources/actuator_nets/abd_act_net.pt"
            abd_actuator_network = torch.jit.load(abd_actuator_path).to(self.device)
            hip_actuator_path = "/home/xterra/walk-these-ways/resources/actuator_nets/hip_act_net.pt"
            hip_actuator_network = torch.jit.load(hip_actuator_path).to(self.device)
            knee_actuator_path = "/home/xterra/walk-these-ways/resources/actuator_nets/abd_act_net.pt"
            knee_actuator_network = torch.jit.load(knee_actuator_path).to(self.device)
            def eval_actuator_network(joint_pos, joint_pos_last, joint_pos_last_last, 
                ...
                torques = torch.zeros_like(joint_pos)
                ...
                # Process each joint type with its corresponding network
                # Abduction joints
                abd_xs = xs[:, abd_indices].reshape(-1, 6)
                abd_torques = abd_actuator_network(abd_xs)
                torques[:, abd_indices] = -abd_torques.view(-1, len(abd_indices))
                
                # Hip joints
                hip_xs = xs[:, hip_indices].reshape(-1, 6)
                hip_torques = 1.5*hip_actuator_network(hip_xs)
                torques[:, hip_indices] = -hip_torques.view(-1, len(hip_indices))
                
                # Knee joints
                knee_xs = xs[:, knee_indices].reshape(-1, 6)
                knee_torques = knee_actuator_network(knee_xs)
                torques[:, knee_indices] = -knee_torques.view(-1, len(knee_indices))
                
                return torques
            
            # def eval_actuator_network(joint_pos, joint_pos_last, joint_pos_last_last, joint_vel, joint_vel_last,
            #                           joint_vel_last_last, act_net):
            #     xs = torch.cat((joint_pos.unsqueeze(-1),
            #                     joint_pos_last.unsqueeze(-1),
            #                     joint_pos_last_last.unsqueeze(-1),
            #                     joint_vel.unsqueeze(-1),
